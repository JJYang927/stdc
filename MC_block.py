...
class conv3x3(nn.Module):
    def __init__(self, in_planes, out_planes, stride=1, groups=1):
        ...
        self.register_parameter('gate', nn.Parameter(torch.randn(1, in_planes // groups, 1, 1)))
    ...
    def forward(self, x):
        ...
        sobel_kernel = gate.repeat(self.out_channels, 1, 3, 3)
...
class conv1x1(nn.Module):
    ...
    def forward(self, x):
        gate = self.sigmoid(self.gate)
        sobel_kernel = gate.repeat(self.out_channels, 1, 1, 1)
...
class MCBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(MCBasicBlock, self).__init__()
...
class MCBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(MCBottleneck, self).__init__()
